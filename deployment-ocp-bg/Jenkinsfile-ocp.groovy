...
podTemplate(
	nodeSelector: 'jenkins_slave=true',
	containers: [
		containerTemplate(name: 'jnlp', image: 'image-registry.openshift-image-registry.svc:5000/openshift/jenkins-agent-maven:latest', args: '${computer.jnlpmac} ${computer.name}'),
		containerTemplate(name: 'gradle', image: 'image-registry.openshift-image-registry.svc:5000/cicd/gradle:jdk8', ttyEnabled: true, command: 'cat', envVars: [
                envVar(key: 'GRADLE_USER_HOME', value: '/tmp')
    	]),
		containerTemplate(name: 'podman', image: 'image-registry.openshift-image-registry.svc:5000/cicd/podman:2.0', privileged: true, ttyEnabled: true, command: 'cat')
	],
	//volume mount
	volumes: [
		hostPathVolume(mountPath: '/home/gradle/.gradle', hostPath: '/tmp/.gradle')
	]
) 
{
	node(POD_LABEL) {
		...
		stageName = "Get Source from gitLab"	
		stage(stageName) {
			echo "**** START : " + stageName
			
			git "http://cloudhr:GS5E1YZDTEYgVsLbzzks@192.168.229.153:15100/root/cloudhr-portal.git"
			sh "ls -al"	
		}
		...
		try {	
			...
			stage(stageName) {
				echo "\n******* START : " + stageName + "\n"
				container('gradle') {
					sh "gradle -b build_ocp_nexus.gradle -x test build"
				}
			...
			stage(stageName) {
				...
				container('podman') {
					sh "podman login -u admin  ${internalRegistry} --tls-verify=false -p ${pw}"
